...
方向上变化时才视为保护弱化。前端提示与服务端抢占控制权前的预检共用
`isSafetyReduction()`，避免绕过或两端判断不一致。

- `vehicle_config_set` 只接受固件 profile 声明的字段，参数类型从服务端的已验证参数缓存取得，不信任浏览器输入。
...
- 降低失控保护或关闭全部解锁检查时，浏览器和服务端都要求 `reduce_failsafe_protection` 明示确认。
