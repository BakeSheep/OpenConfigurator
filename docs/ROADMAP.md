# OpenConfigurator 路线图

路线图表达方向而非交付承诺。优先级会依据安全风险、真实硬件反馈和维护资源调整。

## 近期：公开预览版

- 选择并加入明确的开源许可证
- 补充界面截图、演示视频和首个版本发布说明
- 建立 Windows / Linux 的基础 CI：typecheck、server tests、build
- 完成主流 PX4 版本、USB 串口与 Windows Bluetooth SPP 的硬件验证矩阵
- 完善加速度计、陀螺仪、磁力计等分步校准体验
- 整理可公开复现的 Bug 报告模板与兼容设备清单

## 中期：可用性与连接能力

- UDP 连接与 PX4 SITL 支持
- 地图、GPS 轨迹和基础任务规划
- 飞行数据记录、导出与离线回放
- 多语言与无障碍体验
- 大型参数表同步性能、取消和恢复体验优化
- 更完整的遥控器校准流程

## 长期：生态与高级工具

- 安全的固件管理与升级工作流
- 航点任务编辑、校验、上传和下载
- 插件化仪表与诊断面板
- 可复用的协议兼容测试工具与硬件实验室矩阵

## 当前非目标

- 不宣称替代经过大量实机验证的成熟 GCS。
- 不在缺少纵深防御的情况下提供互联网直连飞控。
- 不为演示效果绕过电机、解锁、手柄或控制权保护。
- 不把尚未接入的按钮、页面或静态数据描述为已实现功能。

提出新方向前请先阅读 [CONTRIBUTING.md](../CONTRIBUTING.md)，并在 Issue 中说明使用场景、风险和验证计划。
