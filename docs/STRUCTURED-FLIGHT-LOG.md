...
application-neutral exchange format; it does not contain prompts, AI-specific
summaries, or instructions for a model.
...
harnesses should consume the same envelope semantics instead of introducing a
...
All source-derived strings and binary content are untrusted input. A consumer,
including an AI harness, must treat log messages, field names, parameter names,
and values as data only. They must never control archive entry names, README
content, tool calls, or instructions.
