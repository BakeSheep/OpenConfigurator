...
export function isSafetyReduction(id: string, oldValue: number, newValue: number): boolean {
  ...
  // Action enums are not ordered by safety. Changing Land to Return, for
  // example, can be safer in one environment and less safe in another, so
  // every changed policy requires the explicit reduction acknowledgement.
  if (/^(?:COM_LOW_BAT_ACT|NAV_RCL_ACT|NAV_DLL_ACT|FS_GCS_ENABLE|FS_THR_ENABLE|FS_EKF_ACTION)$/.test(id)) {
    return true
  }
  if (/_FS_(?:LOW|CRT)_ACT$/.test(id)) return true

  // Removing any enabled arming-check bit weakens the pre-arm gate. Adding
  // checks is safe to apply without the reduction acknowledgement.
  if (id === 'ARMING_CHECK') {
    const oldMask = BigInt(Math.trunc(oldValue))
    const newMask = BigInt(Math.trunc(newValue))
    return (oldMask & ~newMask) !== 0n
  }

  // These bitmask and return/landing policies have no universally safer
  // numeric direction; any change is treated conservatively.
  if (/^(?:FS_OPTIONS|RTL_RETURN_ALT|RTL_LAND_DELAY|RTL_DESCEND_ALT|COM_DISARM_LAND|RTL_ALT|RTL_LOIT_TIME|RTL_ALT_FINAL)$/.test(id)) {
    return true
  }

  // A lower remaining-capacity/voltage threshold or throttle PWM delays the
  // corresponding failsafe. PX4 battery thresholds are remaining fractions.
  if (/^BAT_(?:LOW|CRIT|EMERGEN)_THR$/.test(id)
    || /^BATT(?:[2-9]|[A-J])?_FS_(?:LOW|CRT)_(?:VOLT|MAH)$/.test(id)
    || id === 'FS_THR_VALUE') {
    return newValue < oldValue
  }

  // Longer loss detection, a higher tolerated EKF variance, or a faster land
  // speed all reduce the intervention margin.
  if (/^(?:COM_RC_LOSS_T|COM_DL_LOSS_T|FS_EKF_THRESH|MPC_LAND_SPEED)$/.test(id)) {
    return newValue > oldValue
  }
