...
export function parameterEnumOptions(id: string, identity: VehicleIdentity | null): ParameterEnumOption[] | null {
  if (identity?.family === 'px4') return catalogOptions(PX4_CATALOG, id) ?? LEGACY_PX4_ENUMS[id] ?? null
  if (identity?.family === 'ardupilot' && identity.vehicleClass === 'copter') return catalogOptions(ARDUCOPTER_CATALOG, id)
  return null
}
...
export function parameterEnumLabel(id: string, value: number, identity: VehicleIdentity | null): string | null {
