...
async function testDuplicateChunkFloodStalls(): Promise<void> {
  ...
  const payload = Buffer.alloc(LOG_DATA_CHUNK_SIZE, 0x5a)
  transport.responder = (request) => {
    if (request.kind === 'list') {
...
async function testPassDeadlineEndsQuietPasses(): Promise<void> {
  ...
  const content = Buffer.alloc(LOG_DATA_CHUNK_SIZE * 5)
 ...
 * next attempt retries and can succeed. The blocker is a regular file at the
 * configured path and must not be deleted by the failed attempt.
 */
async function testEnsureDirFailureRetries(): Promise<void> {
  const base = await fsp.mkdtemp(path.join(os.tmpdir(), 'oc-logxfer-dirprep-'))
  const blocked = path.join(base, 'blocked')
  await fsp.writeFile(blocked, 'placeholder')
  const { transport, service } = await makeService({ downloadDir: blocked })
  const content = Buffer.from('retry-payload')
  serveLog(transport, 31, content)

  service.startDownload(31, 'attempt-1')
  await waitFor(() => transport.messagesOfType('log_op_error').length === 1)
  const failure = transport.messagesOfType('log_op_error')[0]
  assert.equal(failure.data.operation, 'download')
  assert.equal(failure.data.code, 'log_transfer_internal')
  assert.equal(
  ...
  service.startDownload(31, 'attempt-2')
  await waitFor(() => transport.messagesOfType('log_download_complete').length === 1)
  const complete = transport.messagesOfType('log_download_complete')[0]
  const record = service.getDownload(complete.data.downloadId)
  assert.ok(record)
...
async function testDefaultPrivateDirIsolated(): Promise<void> {
  ...
  serveLog(transport, 41, Buffer.from('private-dir-check'))
...
async function testCleanupOnlyTouchesOwnArtifacts(): Promise<void> {
  ...
  serveLog(transport, 51, Buffer.from('selective-cleanup'))
...
async function main(): Promise<void> {
  ...
  await testEnsureDirFailureRetries()
...
await (async () => {
  ...
  transport.responder = (request) => {
    ...
    if (request.kind === 'data') {
      const chunk = Buffer.alloc(LOG_DATA_CHUNK_SIZE, 0x6b)
