...
export class MavlinkBridge extends EventEmitter {
  ...
  private conflictAllowsMessage(msg: ClientMessage): boolean {
    if (msg.type === 'select_target' || msg.type === 'release_control') return true
  ...
  private deliverShellMessage(message: ServerMessage): void {
  ...
  // WS client so shell sessions can be pinned to an owner (OCSA-007).
  handleClientMessage(msg: ClientMessage, ownerMeta?: { clientId?: string }): MavlinkBridgeClientResult {
