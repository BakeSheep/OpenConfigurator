...
function startMessage(mode: SafetyMode): Extract<Parameters<EscService['handleClientMessage']>[1], { type: 'esc_session_start' }> {
  return {
    type: 'esc_session_start',
    safetyConfirmation: ESC_SESSION_SAFETY_CONFIRMATION,
    ...TEST_SAFETY_EXPECTATION,
    data: mode === 'px4_serial_control'
      ? { mode, channels: [20, 21] }
      : { mode },
  } as Extract<Parameters<EscService['handleClientMessage']>[1], { type: 'esc_session_start' }>
}

function lastOpErrorCode(emitted: ServerMessage[]): string | undefined {
  for (let i = emitted.length - 1; i >= 0; i--) {
...
async function startSession(
  ...
  await harness.service.handleClientMessage(clientId, startMessage(mode))
  await wait(10)
...
async function safetyContextTests(): Promise<void> {
  ...
  for (const mode of ['ardupilot_passthrough', 'px4_serial_control'] as const) {
    for (const [label, overrides, expectedCode] of [
      ...
      await harness.service.handleClientMessage('client-a', startMessage(mode))
  ...
  // A missing snapshot fails closed as well.
  {
    ...
    await harness.service.handleClientMessage('client-a', startMessage('ardupilot_passthrough'))
  ...
  // is terminated, releasing the borrowed transport.
  {
    ...
    await harness.service.handleClientMessage('client-a', {
  ...
  // place; an explicit reconnect (fresh connection history) may enter.
  {
    ...
    await harness.service.handleClientMessage('client-a', startMessage('direct'))
