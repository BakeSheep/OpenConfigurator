...
export function createApp(options: CreateAppOptions = {}): BackendRuntime {
  ...
  function releaseController(context: ClientContext, reason: 'released' | 'disconnected'): boolean {
    ...
    if (radioControllerPin?.clientId === context.id) return false
    controllerLease = null
  ...
  const onBridgeMessage = (rawMessage: unknown): void => {
    ...
    if (message.type === 'rc_channels') {
      radioManager?.handleRcChannels(message.data as import('../shared/types').RcChannelsData)
    }
    if (message.type === 'airframe_apply_status' && message.data.phase === 'rebooting') {
  ...
  const onVehicleReadyChange = (ready: boolean): void => {
    ...
    if (!ready && connManager.rawSessionActive !== true) radioManager.handleLinkDown()
  ...
  radioManager = new RadioCalibrationSessionManager({
    ...
    applyCalibration: (requestId, channels, mapped, completion) => {
      const apply = mavlinkBridge.applyRadioCalibration
      if (!apply) {
        completion(false, 'unsupported_operation')
        return
      }
      apply.call(mavlinkBridge, requestId, channels, mapped, completion)
    },
