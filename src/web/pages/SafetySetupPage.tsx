...
export default function SafetySetupPage() {
  ...
  const fields = useMemo(() => {
    ...
    if (identity?.family === 'ardupilot') {
      for (const id of [...params.keys()].filter((id) => /^BATT(?:[2-9]|[A-J])?_FS_(?:LOW|CRT)_(?:VOLT|MAH|ACT)$/.test(id)).sort()) {
  ...
  if (!caps.safetyConfig) return <SetupNotice state="warning">{t('vehicleSetup.readOnlyProfile')}</SetupNotice>
  return (
    <div className="mc-setup-page mc-fade-in">
      <section className="mc-card mc-safety-hero" data-state={status?.failsafe ?? 'unknown'}>
        <div><span className="mc-eyebrow">{t('vehicleSetup.safetyOverview')}</span><h2>{t(`vehicleSetup.failsafeState.${status?.failsafe ?? 'unknown'}`)}</h2><p>{t('vehicleSetup.safetyHint')}</p></div>
        <span className="mc-safety-orbit" aria-hidden="true"><i /><i /><i /></span>
      </section>
      ...
      {[...groups].map(([group, groupFields]) => (
        <section className="mc-card mc-setup-panel" key={group}>
          <header><div><h2>{t(`vehicleSetup.group.${group}`, { defaultValue: group })}</h2><p>{t('vehicleSetup.safetyGroupHint')}</p></div><span className="mc-setup-panel__count">{groupFields.length}</span></header>
          <div className="mc-setup-fields">
            {groupFields.map((field) => <ConfigFieldControl
              key={field.id}
              feature="safety"
              field={field}
      ...
      {fields.length === 0 && !loading && <SetupNotice state="warning">{t('vehicleSetup.parametersMissing')}</SetupNotice>}
    </div>
  )
}
