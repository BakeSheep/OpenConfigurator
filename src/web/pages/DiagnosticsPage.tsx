...
type DiagnosticSection = 'parameters' | 'pid' | 'ekf' | 'waveforms' | 'messages'
  ...
  return (
    <div className="mc-workspace mc-workspace--full mc-fade-in">
      ...
      <div className="mc-subworkspace">
        <nav className="mc-subnav" aria-label="调参与诊断">
          {sections.map((section) => (
            <button key={section.id} type="button" data-active={section.id === activeSection} aria-current={section.id === activeSection ? 'page' : undefined} onClick={() => setSearchParams(section.id === 'parameters' ? {} : { section: section.id }, { replace: true })}>
