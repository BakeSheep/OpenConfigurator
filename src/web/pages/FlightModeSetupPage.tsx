...
function ModeMaskControl({ field, label }: { field: VehicleConfigField; label: string }) {
  ...
  const toggle = (slot: number) => sendClientMessage({
    type: 'vehicle_config_set', requestId: setupRequestId(`mask-${field.id}`), feature: 'flight_modes',
    data: { id: field.id, value: value ^ (1 << slot) },
  })
  return <div className="mc-setup-field mc-mode-mask"><label><span>{label}</span><code>{field.id}</code></label><div role="group" aria-label={label}>{Array.from({ length: 6 }, (_, slot) => <button type="button" key={slot} aria-pressed={Boolean(value & (1 << slot))} data-active={Boolean(value & (1 << slot))} disabled={!vehicleReady || !canControl || armed !== false} onClick={() => toggle(slot)}>{t('vehicleSetup.modeSlotShort', { slot: slot + 1 })}</button>)}</div></div>
}
...
export default function FlightModeSetupPage() {
  ...
  const fields = useMemo(() => {
    ...
    if (identity?.family === 'ardupilot') {
      for (const id of [...params.keys()].filter((id) => /^(?:RC(?:[6-9]|1[0-6])_OPTION|CH(?:[7-9]|1[0-6])_OPT)$/.test(id)).sort()) {
  ...
  } else if (identity?.family === 'ardupilot') activeSlot = arduFlightModeSlot(pwm)
  ...
  const conflicts = channelAssignments.filter((entry, index, all) => all.findIndex((candidate) => candidate.channel === entry.channel) !== index)
  const knownModes = availableModes(identity)

  if (!caps.flightModeConfig) return <SetupNotice state="warning">{t('vehicleSetup.readOnlyProfile')}</SetupNotice>
  return (
    <div className="mc-setup-page mc-fade-in">
      <section className="mc-card mc-mode-live">
        <header>
          <div><span className="mc-eyebrow">{t('vehicleSetup.liveSwitch')}</span><h2>{t('vehicleSetup.sixPositionModes')}</h2></div>
        ...
        <div className="mc-mode-slots" role="list" aria-label={t('vehicleSetup.sixPositionModes')}>
          {Array.from({ length: 6 }, (_, index) => {
            ...
            const label = value === undefined ? '—' : parameterEnumLabel(id, value, identity)
              ?? knownModes.find((mode) => mode.id === value)?.name
              ?? `${t('vehicleSetup.unknownValue')} ${value}`
            return <div key={id} className="mc-mode-slot" data-active={activeSlot === index} role="listitem"><span>{index + 1}</span><strong>{label}</strong><small>{id}</small></div>
          })}
        </div>
      </section>
      ...
      {conflicts.length > 0 && <SetupNotice state="warning">{t('vehicleSetup.channelConflict', { channels: [...new Set(conflicts.map((entry) => `CH${entry.channel}`))].join(', ') })}</SetupNotice>}
      ...
      {identity?.family === 'px4' && channelAssignments.some((entry) => entry.id !== 'RC_MAP_FLTMODE') && (
        <section className="mc-switch-live-grid" aria-label={t('vehicleSetup.dedicatedSwitchState')}>
          {channelAssignments.filter((entry) => entry.id !== 'RC_MAP_FLTMODE').map((entry) => {
            ...
            return <div className="mc-card" key={entry.id} data-active={live !== null && live >= 1500}><code>{entry.id}</code><strong>CH{entry.channel}</strong><small>{live ?? '—'} µs</small></div>
          })}
        </section>
      )}
      ...
      {[...groups].map(([group, groupFields]) => (
        <section className="mc-card mc-setup-panel" key={group}>
          <header><div><h2>{t(`vehicleSetup.group.${group}`, { defaultValue: group })}</h2><p>{t('vehicleSetup.immediateWriteHint')}</p></div></header>
          <div className="mc-setup-fields">
            {groupFields.map((field) => field.id === 'SIMPLE' || field.id === 'SUPER_SIMPLE'
