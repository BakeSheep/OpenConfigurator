...
export default function GamepadVisualizer({
  ...
  const buttonFill = (index: number) => buttons[index] ? 'var(--mc-color-accent-solid)' : 'var(--bg-hover)'
  ...
  return (
    <div className="relative flex min-h-[320px] items-center overflow-hidden rounded-2xl border p-3" style={{ background: 'linear-gradient(145deg, var(--bg-tertiary), var(--bg-secondary))', borderColor: 'var(--border)' }}>
        <div className="absolute left-4 top-4 z-10 flex items-center gap-2 text-[11px] font-bold tracking-[0.12em]" style={{ color: connected ? 'var(--success)' : 'var(--text-secondary)' }}>
          ...
          {connected ? t('joystick.visualizer.connected') : t('joystick.visualizer.noDevice')}
        </div>
        <svg viewBox="0 0 560 300" className="h-auto w-full" role="img" aria-label={t('joystick.visualizer.aria.gamepadState')}>
          ...
          {[
            ...
            <g key={stick.label}>
              ...
              <circle cx={stick.cx + stick.x * 19} cy={stick.cy + stick.y * 19} r="20" fill={connected ? 'var(--mc-color-accent-solid)' : 'var(--bg-hover)'} stroke={connected ? 'var(--accent-hover)' : 'var(--border-strong)'} strokeWidth="2" style={{ transition: 'cx 40ms linear, cy 40ms linear' }} />
              <text x={stick.cx + stick.x * 19} y={stick.cy + stick.y * 19 + 4} textAnchor="middle" fontSize="10" fontWeight="800" fill={connected ? 'var(--mc-color-on-accent)' : 'var(--text-secondary)'}>{stick.label}</text>
            </g>
          ))}
          ...
          {[
            ...
            <g key={button.index}>
              ...
              <text x={button.cx} y={button.cy + 4} textAnchor="middle" fontSize="10" fontWeight="800" fill={buttons[button.index] ? 'var(--mc-color-on-accent)' : button.color}>{button.label}</text>
            </g>
          ))}
