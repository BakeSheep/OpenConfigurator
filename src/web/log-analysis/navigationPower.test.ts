...
describe('powerModule', () => {
  ...
  it('produces voltage and current chart series', async () => {
    ...
    const batteryFamily = result.chartFamilies.find(f => f.id === 'battery')
    assert.ok(batteryFamily, 'should have battery chart family')
    const voltageChart = batteryFamily!.views.find(v => v.id === 'battery-voltage')
    const currentChart = batteryFamily!.views.find(v => v.id === 'battery-current')
    assert.ok(voltageChart, 'should have voltage view')
    assert.ok(currentChart, 'should have current view')
    assert.equal(voltageChart!.unit, 'V')
    assert.equal(currentChart!.unit, 'A')
...
describe('propulsionModule', () => {
  ...
  it('produces RPM chart series', async () => {
    ...
    const propulsionFamily = result.chartFamilies.find(f => f.id === 'propulsion')
    assert.ok(propulsionFamily, 'should have propulsion chart family')
    const rpmChart = propulsionFamily!.views.find(v => v.id === 'esc-rpm')
    assert.ok(rpmChart, 'should have RPM view')
...
describe('navigationModule', () => {
  ...
  it('produces navigation chart series', async () => {
    ...
    const gpsFamily = result.chartFamilies.find(f => f.id === 'gps-quality')
    assert.ok(gpsFamily, 'should have GPS quality chart family')
    const fixChart = gpsFamily!.views.find(v => v.id === 'gps-fix-type')
    assert.ok(fixChart, 'should have GPS fix quality view')
...
describe('eventsModule', () => {
  ...
  it('finalizes with empty result when no events consumed', () => {
    ...
    assert.equal(result.chartFamilies.length, 0)
