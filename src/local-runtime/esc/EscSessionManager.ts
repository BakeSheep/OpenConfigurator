...
import { randomBase64Url, randomUUID } from '../platform/crypto'
import {
  EscError,
  toEscError,
  type EscJobKind,
...
export interface EscSessionManagerOptions {
  ...
  /** Bind local control authority to the active ESC session owner. */
  pinController?: (ownerClientId: string, sessionId: string) => void
  /** Release the session-bound local authority when the session ends. */
...
export class EscSessionManager extends EventEmitter {
  ...
  async start(
    ...
    const session: InternalSession = {
      ...
      ownerClientId,
      target,
      recoveryToken: randomBase64Url(24),
      safetyConfirmed,
      transport,
      abort,
      state: 'entering',
