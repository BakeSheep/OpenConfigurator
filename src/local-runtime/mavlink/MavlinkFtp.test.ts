...
// and single-task mutual exclusion. Run directly: tsx src/local-runtime/mavlink/MavlinkFtp.test.ts
...
function px4ReferenceCrc32(data: ByteBuffer): number {
  let crc = 0
...
assert.equal(crc32Buffer(ByteBuffer.alloc(0)), 0)
assert.equal(crc32Buffer(ByteBuffer.from('123456789')), 0x2dfd2d88)
assert.equal(crc32Buffer(ByteBuffer.from('123456789')), px4ReferenceCrc32(ByteBuffer.from('123456789')))
...
function parseRequest(payload: ByteBuffer): ParsedRequest {
  return {
...
function buildReply(options: {
  ...
  const payload = ByteBuffer.alloc(12 + data.length)
...
function nakData(code: number): ByteBuffer {
  return ByteBuffer.from([code])
}
...
class FakeTransport implements FtpTransport {
  ...
  messagesOf<T extends RuntimeEvent['type']>(type: T): Array<Extract<RuntimeEvent, { type: T }>> {
 ...
 */
function serveTinyDownload(transport: FakeTransport, content: ByteBuffer, session = 4): void {
...
await (async () => {
  ...
  const pages: Array<ByteBuffer | null> = [
    ByteBuffer.from('D2026-01-01\0Ffoo.ulg\t1234\0S\0', 'utf8'),
    ByteBuffer.from('Fbar.ulg\t99\0D.\0D..\0', 'utf8'),
    null, // EOF
  ]
  transport.responder = (request) => {
...
await (async () => {
  ...
  const content = ByteBuffer.alloc(fileSize)
  ...
  transport.responder = (request) => {
    switch (request.opcode) {
      ...
      case FTP_OPCODES.OpenFileRO: {
        ...
        const sizeBuf = ByteBuffer.alloc(4)
  ...
  const record = ftp.getDownload(complete.data.artifactId)
  assert.ok(record)
  assert.equal(record!.sizeBytes, fileSize)
  const written = await fsp.readFile(record!.filePath)
  assert.deepEqual(ByteBuffer.from(written), content, 'downloaded bytes must match the source file')
...
await (async () => {
  const { transport, ftp, dir } = await makeFtp()
  const content = ByteBuffer.from('crc-corruption-check')
  const session = 9
  transport.responder = (request) => {
    switch (request.opcode) {
      case FTP_OPCODES.ResetSessions:
      ...
      case FTP_OPCODES.ReadFile:
        ...
        break
      case FTP_OPCODES.CalcFileCRC32: {
        const wrongCrc = ByteBuffer.alloc(4)
    ...
    switch (request.opcode) {
      ...
      case FTP_OPCODES.BurstReadFile:
        ...
        transport.reply(request, {
          ...
          session,
          offset: 0,
          data: ByteBuffer.alloc(200, 0xaa),
          reqOpcode: FTP_OPCODES.BurstReadFile,
        })
  ...
  transport.responder = (request) => {
    switch (request.opcode) {
      case FTP_OPCODES.ListDirectory: {
        ...
        transport.reply(request, {
          opcode: FTP_OPCODES.Ack,
          data: ByteBuffer.from('Fa.ulg\t10\0Fb.ulg\t20\0', 'utf8'),
        })
        break
      }
...
await (async () => {
  const { transport, ftp, dir } = await makeFtp()
  const session = 5
  const noise = ByteBuffer.alloc(200, 0x77)
  let floodSeq = 100
...
await (async () => {
  ...
  transport.responder = (request) => {
    ...
    transport.reply(request, {
      opcode: FTP_OPCODES.Ack,
      data: ByteBuffer.from('S\0'.repeat(50), 'utf8'),
    })
  }
  ftp.startList('/fs/microsd/log')
  await waitFor(() => transport.messagesOf('fs_op_error').length === 1)
...
await (async () => {
  ...
  transport.ftp = ftp
  serveTinyDownload(transport, ByteBuffer.from('private-dir-check'))
...
await (async () => {
  ...
  // The flat artifact store has no empty directories; a nested foreign file
  // stands in for "content outside the download directory's top level".
  await fsp.writeFile(path.join(dir, 'subdir', 'keep.log'), 'nested foreign file')
  serveTinyDownload(transport, ByteBuffer.from('selective-cleanup'))
  ...
  assert.equal(
    new TextDecoder().decode(await fsp.readFile(path.join(dir, 'subdir', 'keep.log'))),
    'nested foreign file',
    'nested foreign content must survive cleanup',
  )
...
// ---------------------------------------------------------------------------
// OCSA-008: crash-leftover instance directories are reclaimed from the shared
// parent. The browser-local artifact store hosts a single runtime instance, so
// every prefixed directory is a leftover; unrelated names are never touched.
// ---------------------------------------------------------------------------
await (async () => {
  ...
  assert.deepEqual(removed.sort(), [
    `${FTP_DOWNLOAD_DIR_PREFIX}ancient`,
    `${FTP_DOWNLOAD_DIR_PREFIX}fresh`,
  ])
  assert.equal(
    new TextDecoder().decode(await fsp.readFile(plainFile)),
    'not a download directory',
    'entries without the instance prefix must survive the sweep',
  )
    ...
    switch (request.opcode) {
      case FTP_OPCODES.ListDirectory: {
        ...
        const page = dirPath === '/fs/microsd/log/danger'
          ...
          : ByteBuffer.alloc(0) // okdir is empty
        transport.reply(request, { opcode: FTP_OPCODES.Ack, data: page })
        break
      }
      case FTP_OPCODES.RemoveFile:
...
await (async () => {
  const { transport, ftp, dir } = await makeFtp()
  const session = 9
  const chunk = ByteBuffer.alloc(1, 0x5a)
