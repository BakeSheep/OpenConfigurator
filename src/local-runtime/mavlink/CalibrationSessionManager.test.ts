...
import type { CalibrationSnapshot, RuntimeEvent } from '../../shared/types'
import {
...
type Harness = {
  ...
  perClient: Array<{ clientId: string; message: RuntimeEvent }>
...
function makeManager(): Harness {
  ...
  const broadcasts: RuntimeEvent[] = []
  const perClient: Array<{ clientId: string; message: RuntimeEvent }> = []
...
// -- start: owner notification, pinning, single active session ------------------
{
  const h = makeManager()
  ...
  // Internal recovery details must never appear in any page event.
...
// -- terminal: pin released, retention window, replay ------------------------------
{
  ...
  const afterRetention: RuntimeEvent[] = []
