...
import { artifactPath as path } from '../platform/artifactFs'
...
/**
 * Remove leftover instance directories carrying the given prefix from
 * `parent`. The browser-local artifact store hosts a single runtime instance,
 * so every matching directory is by definition a crashed-run leftover and is
 * removed unconditionally. Returns the names that were removed.
 */
export async function removeStaleInstanceDirs(
  ...
  const names = await artifactFs.readdir(parent).catch(() => [] as string[])
  for (const name of names) {
    if (!name.startsWith(prefix)) continue
    await artifactFs.rm(path.join(parent, name), { recursive: true, force: true }).catch(() => undefined)
    removed.push(name)
  }
  return removed
}
...
/**
 * Mirrors the runtime boundary's devicePath() contract (local-runtime
 * validation): absolute, bounded by FTP_MAX_PATH_BYTES, free of control
 * characters and free of `..` segments. Applied a second time after joining
 * remote listing names so a composed delete target can never escape the
...
export async function assertDownloadCapacity(_directory: string, sizeBytes: number): Promise<void> {
  ...
  const { quota, usage } = await artifactFs.estimate()
  const availableBytes = Math.max(0, quota - usage)
  const requiredBytes = sizeBytes + MIN_FREE_SPACE_AFTER_DOWNLOAD_BYTES
