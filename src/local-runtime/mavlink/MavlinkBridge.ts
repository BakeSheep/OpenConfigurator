...
import type {
  SerialWritePriority,
  SerialWriteQueueTag,
} from '../connection/BrowserConnectionManager'
...
import type { RuntimeEvent, RuntimeCommand, ManualControlData, MessageRateConfig, RcChannelsData, RadioCalibrationChannel } from '../../shared/types'
import {
...
export class MavlinkBridge extends EventEmitter {
  ...
  // Active calibration session, if any. Owned lifecycle-wise by the local
  ...
  private onData = (data: ByteBuffer) => {
  ...
  constructor(connManager: BrowserConnectionManager, options: MavlinkBridgeOptions = {}) {
    super()
  ...
  private hasReadyTarget(): boolean {
    const managerReady = (this.connManager as BrowserConnectionManager & {
  ...
  private buildTargetMessage(reason: 'discovered' | 'selected' | 'reset'):
    Extract<RuntimeEvent, { type: 'target' }> {
    return {
      type: 'target',
      data: {
  ...
  /**
   * Return the current target state without emitting it. The local runtime
  ...
  private handleSysStatus(msg: MavlinkMessage) {
    ...
    if (!d) return
    // These are bitmask fields; generated dialect mapping types them as the sensor enum, so
  ...
  private handleGps(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'telemetry', msgType: 'GPS_RAW_INT', data } as RuntimeEvent)
  }
  ...
  private handleScaledImu(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'sensor', msgType: msg.msgId === 116 ? 'SCALED_IMU2' : msg.msgId === 129 ? 'SCALED_IMU3' : 'SCALED_IMU', data } as RuntimeEvent)
  }
  ...
  private handleHighresImu(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'sensor', msgType: 'HIGHRES_IMU', data } as RuntimeEvent)
  ...
  private handleRawImu(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'sensor', msgType: 'RAW_IMU', data } as RuntimeEvent)
  }
  ...
  private handleScaledPressure(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'sensor', msgType: 'SCALED_PRESSURE', data } as RuntimeEvent)
  }
  ...
  private handleAttitude(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'telemetry', msgType: 'ATTITUDE', data } as RuntimeEvent)
  }
  ...
  private handleGlobalPosition(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'telemetry', msgType: 'GLOBAL_POSITION_INT', data } as RuntimeEvent)
  }

  private handleRcChannels(msg: MavlinkMessage) {
    const d = decode<common.RcChannels>(65, msg.payload)
    if (!d) return
    // generated dialect mapping decodes the size-grouped wire layout (18 uint16 channels
    ...
    this.emit('message', { type: 'rc_channels', data } as RuntimeEvent)
  }
  ...
  private handleVfrHud(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'telemetry', msgType: 'VFR_HUD', data } as RuntimeEvent)
  }
  ...
  private handleCommandAck(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'command_ack', data } as RuntimeEvent)
  ...
  private handleOpticalFlow(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'sensor', msgType: 'OPTICAL_FLOW_RAD', data } as RuntimeEvent)
  }
  ...
  private handleOpticalFlowLegacy(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'sensor', msgType: 'OPTICAL_FLOW', data } as RuntimeEvent)
  }
  ...
  private handleFileTransferProtocol(msg: MavlinkMessage) {
    ...
    this.ftp.handleFtpPayload(ByteBuffer.from(d.payload as unknown as number[]))
  }

  private sendFtpPayload(payload: ByteBuffer): boolean {
    ...
    const padded = ByteBuffer.alloc(251)
  ...
  async readLocalArtifact(
    artifactId: string,
    consume = true,
  ): Promise<{ fileName: string; data: Uint8Array }> {
    const record = consume
      ? this.ftp.consumeDownload(artifactId) ?? this.logTransfer.consumeDownload(artifactId)
      : this.getFtpDownload(artifactId)
    if (!record) throw new Error('本地日志文件不存在或已过期')
    const data = await artifactFs.readFile(record.filePath)
    if (consume) await artifactFs.unlink(record.filePath)
    return { fileName: record.fileName, data }
  }
  ...
  private handleLogData(msg: MavlinkMessage) {
    ...
    this.logTransfer.handleLogData({
      ...
      ofs: d.ofs,
      count: d.count,
      data: ByteBuffer.from(d.data as unknown as number[]),
    })
  }
  ...
  sendSerialControl(fields: {
    ...
    const padded = ByteBuffer.alloc(70)
    ByteBuffer.from(fields.data).copy(padded, 0)
  ...
  private sendShellPayload(text: string): boolean {
    const bytes = ByteBuffer.from(text, 'utf8')
  ...
  private handleDistanceSensor(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'sensor', msgType: 'DISTANCE_SENSOR', data } as RuntimeEvent)
  }
  ...
  private handleRangefinder(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'sensor', msgType: 'RANGEFINDER', data } as RuntimeEvent)
  }
  ...
  private handleBattery(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'telemetry', msgType: 'BATTERY_STATUS', data } as RuntimeEvent)
  }
  ...
  private handleEstimatorStatus(msg: MavlinkMessage) {
    ...
    this.emit('message', { type: 'ekf_status', data } as RuntimeEvent)
  }
  ...
  private handleStatustext(msg: MavlinkMessage) {
    ...
    const payload = msg.payload.length >= 54
      ? msg.payload
      : ByteBuffer.concat([msg.payload, ByteBuffer.alloc(54 - msg.payload.length)])
    const severity = payload[0]
    const rawText = payload.subarray(1, 51)
    const terminatorIndex = rawText.indexOf(0)
    const textBytes = ByteBuffer.from(rawText.subarray(
      0,
    ...
    if (chunkComplete) {
      this.statustextChunks.delete(key)
      this.emitStatustext(pending.severity, ByteBuffer.concat(pending.chunks, pending.byteLength))
    }
  }
  ...
  private emitStatustext(severity: number, bytes: ByteBuffer): void {
  ...
  handleRuntimeCommand(msg: RuntimeCommand, ownerMeta?: { clientId?: string }): MavlinkBridgeClientResult {
    ...
    switch (msg.type) {
      ...
      case 'radio_calibration_cancel':
        // Radio calibration is owned by the Worker-level session manager,
  ...
  private emitVehicleConfigResult(
    requestId: string,
    feature: Extract<RuntimeCommand, { type: 'vehicle_config_set' }>['feature'],
    id: string,
    accepted: boolean,
  ...
  /** Bridge-side profile/arming gate before a local radio session is created. */
  ...
  private setVehicleConfiguration(msg: Extract<RuntimeCommand, { type: 'vehicle_config_set' }>): void {
  ...
  private emitAirframeStatus(
    requestId: string,
    phase: Extract<RuntimeEvent, { type: 'airframe_apply_status' }>['data']['phase'],
    completed: number,
    total: number,
    currentId?: string,
    reason?: string,
  ...
  private applyAirframe(msg: Extract<RuntimeCommand, { type: 'airframe_apply' }>): void {
    const total = 2
    if (!this.requireReadyTarget(msg.type, msg.requestId) || !this.requireWritableVehicle(msg.type, msg.requestId)) return
  ...
  private validateParamSet(id: string, value: number, paramType: number): string | null {
    if (!/^[\x20-\x7e]{1,16}$/.test(id) || ByteBuffer.byteLength(id, 'ascii') > 16) {
  ...
  private transmitPendingParamSet(pending: PendingParamSet): void {
    const valueBuf = ByteBuffer.alloc(4)
  ...
  private handleAutopilotVersion(msg: MavlinkMessage) {
    ...
    // capabilities is a uint64 bitmask (bigint at runtime); generated dialect mapping types
    ...
    const versionMessage = {
      ...
      data: {
        ...
        // 56/58 (inside os_custom_version) - a latent bug fixed by generated dialect mapping.
        vendorId: d.vendorId,
  ...
  getMessageRatesMessage(): Extract<RuntimeEvent, { type: 'message_rates' }> {
  ...
  private decodeParamValue(payload: ByteBuffer, paramType: number) {
  ...
  private writeParamValue(payload: ByteBuffer, value: number, paramType: number) {
  ...
  private sendManualControl(data: ManualControlData) {
    ...
    this.manualControlFlushHandle = setTimeout(() => {
      this.manualControlFlushHandle = null
