...
export class RadioCalibrationSessionManager {
  ...
  handleRcChannels(data: RcChannelsData): void {
    const context = this.options.getVehicleContext()
    if (!context?.ready || context.armed !== false) {
      this.clearRcSamples()
      return
    }
  ...
  requestStart(clientId: string, message: { requestId: string; data: { transmitterMode: 1 | 2 | 3 | 4 } }): void {
    ...
    if (!context?.ready || context.armed !== false) {
      return this.error(
        ...
        context?.armed === true ? 'vehicle_armed' : 'safety_context_unavailable',
        context?.armed === true ? '飞控已解锁，不能校准遥控器' : '飞控目标尚未就绪或上锁状态未知',
      )
    }
    if (this.latestAt === null || this.now() - this.latestAt > RC_SAMPLE_MAX_AGE_MS) {
      this.clearRcSamples()
      return this.error(clientId, message.requestId, 'radio_calibration_start', 'rc_sample_stale', '尚未收到当前飞控的实时遥控器输入')
    }
    ...
    this.active = {
      ...
      recoveryToken: randomBase64Url(32),
  ...
  advance(clientId: string, message: { requestId: string; data: { sessionId: string } }): void {
    ...
    if (!active) return
    if (!this.ensureVehicleContext(active, message.requestId, 'radio_calibration_advance')) return
  ...
  reclaim(clientId: string, message: { requestId: string; data: { sessionId: string; recoveryToken: string } }): void {
    ...
    if (!this.ensureVehicleContext(active, message.requestId, 'radio_calibration_reclaim')) return
    ...
    this.options.emitToClient(clientId, {
      type: 'radio_calibration_started',
      data: { sessionId: active.sessionId, requestId: active.requestId },
    })
    this.publish(active)
  }
  ...
  handleVehicleSafetyBoundary(reason: string): void {
    ...
    active.failureReason = reason === 'vehicle_armed'
      ? '检测到飞控已解锁，遥控器校准已终止'
      : '飞控目标、身份或就绪状态已变化，遥控器校准已终止'
    this.options.notifyCalibration(false)
    this.publish(active)
    this.finish(active)
  }

  handleLinkDown(): void { this.handleVehicleSafetyBoundary('link_lost') }
  ...
  private captureStep(active: ActiveSession, step: RadioCalibrationStep): { code: string; message: string } | null {
    ...
    if (step === 'aux_sweep') {
      const primaryChannels = new Set(Object.values(active.mapped).filter((channel): channel is number => channel !== undefined))
      for (let index = 0; index < active.detectedChannels; index += 1) {
        // Primary axes were already driven to both commanded endpoints in the
        // preceding steps. Requiring every primary channel to repeat that
        // motion during the auxiliary sweep made the review step unreachable.
        if (primaryChannels.has(index + 1)) continue
  ...
  private ensureVehicleContext(active: ActiveSession, requestId: string, operation: string): boolean {
    const context = this.options.getVehicleContext()
    if (
      context?.fingerprint === active.vehicleFingerprint
      && context.ready
      && context.armed === false
    ) return true
    this.error(
      ...
      requestId,
      operation,
      context?.armed === true ? 'vehicle_armed' : 'safety_context_changed',
      context?.armed === true ? '飞控已解锁，校准已终止' : '飞控目标或就绪状态已变化，校准已终止',
    )
    this.handleVehicleSafetyBoundary(context?.armed === true ? 'vehicle_armed' : 'safety_context_changed')
    return false
  }

  private clearRcSamples(): void {
    this.latest = []
    this.latestAt = null
  }
