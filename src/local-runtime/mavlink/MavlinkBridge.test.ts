...
function frameMessageId(frame: ByteBuffer): number {
  return frame[0] === MavLinkProtocolV2.START_BYTE
    ? frame.readUIntLE(7, 3)
    : frame[5]
}

function framePayload(frame: ByteBuffer): ByteBuffer {
...
// ---------------------------------------------------------------------------
{
  ...
  noiseSession.write(ByteBuffer.alloc(1024 * 1024, 0x55))
  ...
  const burst = ByteBuffer.concat(Array.from(
    { length: 300 },
  ...
  const ftpPayload = ByteBuffer.alloc(251, 0x41)
  ...
  noiseSession.write(ByteBuffer.concat([ByteBuffer.from([0xfd, 0xff, 0, 0, 0]), v2Heartbeat]))
  assert.equal(noiseMessages.length, 1)

  const corrupted = ByteBuffer.from(v2Heartbeat)
  corrupted[10] ^= 0x01
  ...
  assert.equal(noiseSession.stats.parserRebuilds, 0)
...
class FakeConnection extends EventEmitter {
  ...
  write(frame: ByteBuffer, priority = 'normal', queueTag?: string): boolean {
...
function commandAckPayload(
  ...
  const payload = ByteBuffer.alloc(10)
...
bridge.handleRuntimeCommand({ type: 'shell_open', requestId: 'shell-open-test' })
...
const shellReply = ByteBuffer.alloc(79)
shellReply[6] = 10
shellReply[7] = 1
shellReply[8] = 5
ByteBuffer.from('nsh> ', 'ascii').copy(shellReply, 9)
...
bridge.handleRuntimeCommand({ type: 'shell_write', data: { text: 'ver hw\r' } })
assert.ok(
...
bridge.handleRuntimeCommand({ type: 'shell_close' })
assert.equal(findLast(messages, (message) => message.type === 'shell_status')?.data.active, false)
...
inject(bridge, 30, ByteBuffer.alloc(28), 43, 1)
assert.equal(connection.activityNotifications, activityBeforeForeignTelemetry)
inject(bridge, 200, ByteBuffer.alloc(4), 43, 1)
assert.equal(connection.activityNotifications, activityBeforeForeignTelemetry)
inject(bridge, 200, ByteBuffer.alloc(4), 42, 1)
assert.equal(connection.activityNotifications, activityBeforeForeignTelemetry + 1)
inject(bridge, 30, ByteBuffer.alloc(28), 42, 1)
...
const rebootResult = bridge.handleRuntimeCommand({
  type: 'reboot_vehicle',
  requestId: 'reboot-1',
...
inject(bridge, 22, ByteBuffer.alloc(24))
...
const euroBytes = ByteBuffer.from('€', 'utf8')
const utf8First = ByteBuffer.alloc(53)
utf8First[0] = 4
ByteBuffer.from('B'.repeat(49), 'ascii').copy(utf8First, 1)
utf8First[50] = euroBytes[0]
utf8First.writeUInt16LE(8, 51)
inject(bridge, 253, utf8First)
const utf8Last = ByteBuffer.alloc(54)
utf8Last[0] = 4
...
const extendedStatePayload = ByteBuffer.from([2, 1])
...
const gpsPayload = ByteBuffer.alloc(30)
...
const batteryPayload = ByteBuffer.alloc(54)
...
const rawImuPayload = ByteBuffer.alloc(29)
...
const flowPayload = ByteBuffer.alloc(44)
...
const flowWithoutGyroPayload = ByteBuffer.from(flowPayload)
...
const legacyFlowPayload = ByteBuffer.alloc(34)
...
const estimatorPayload = ByteBuffer.alloc(44)
...
const distancePayload = ByteBuffer.alloc(14)
...
const rangefinderPayload = ByteBuffer.alloc(8)
...
const invalidPressure = ByteBuffer.alloc(16)
...
bridge.handleRuntimeCommand({ type: 'param_request_list', requestId: 'expired-list' })
bridge.handleRuntimeCommand({ type: 'param_request_list', requestId: 'list-too-large' })
assert.equal(
...
inject(bridge, 0, heartbeatPayload(), 43, 1)
bridge.handleRuntimeCommand({ type: 'param_request_list', requestId: 'switching-list' })
...
// the entire remaining connection lifetime.
{
  ...
  const sendMode = (requestId: string) => ttlBridge.handleRuntimeCommand({
    type: 'command',
    requestId,
    cmd: 'MAV_CMD_DO_SET_MODE',
    params: [1, 4, 3, 0, 0, 0, 0],
...
// cannot extend the fixed transaction deadline forever.
{
  ...
  progressBridge.handleRuntimeCommand({
    type: 'command',
...
// reboot (time_boot_ms regression) re-requests the telemetry intervals.
{
  ...
  const servoPayload = ByteBuffer.alloc(12)
  ...
  const rcPayload = ByteBuffer.alloc(42)
  ...
  const ackPayload = ByteBuffer.alloc(6)
  ...
  const attitudePayload = (bootMs: number) => {
    const payload = ByteBuffer.alloc(28)
...
// to escape into a later/closed physical session.
{
  ...
  destroyBridge.handleRuntimeCommand({
    type: 'manual_control',
...
// firmware as ArduPilot, and never survives a target reset.
{
  ...
  const sysStatusPayload = ByteBuffer.alloc(31)
  ...
  const apBatteryPayload = ByteBuffer.alloc(36)
  ...
  const estimatorPayload = ByteBuffer.alloc(42)
...
// the new (possibly read-only) profile can inherit old commands.
{
  ...
  identityBridge.handleRuntimeCommand({
    type: 'motor_test',
...
// requests must be rejected before serialization - no cross-stack guessing.
{
  ...
  genericBridge.handleRuntimeCommand({
    type: 'set_flight_mode',
  ...
  const genericRebootResult = genericBridge.handleRuntimeCommand({
    type: 'reboot_vehicle',
...
// default-deny write capability as the other unimplemented ArduPilot classes.
{
  ...
  planeBridge.handleRuntimeCommand({ type: 'log_list', requestId: 'plane-list' })
  ...
  planeBridge.handleRuntimeCommand({
    type: 'param_set',
  ...
  const planeRebootResult = planeBridge.handleRuntimeCommand({
    type: 'reboot_vehicle',
...
// ---------------------------------------------------------------------------
{
  ...
  pauseConnection.emit('data', ByteBuffer.from([0x2f, 0x30, 0x00]))
...
// ---------------------------------------------------------------------------
{
  const textPayload = (text: string): ByteBuffer => {
    const payload = ByteBuffer.alloc(54)
    payload[0] = 6
    ByteBuffer.from(text, 'ascii').copy(payload, 1, 0, 50)
    return payload
  }
...
// ---------------------------------------------------------------------------
{
  function statustextPayload(severity: number, text: string): ByteBuffer {
    const payload = ByteBuffer.alloc(54)
    payload[0] = severity
    ByteBuffer.from(text, 'ascii').copy(payload, 1, 0, 50)
    return payload
...
// ---------------------------------------------------------------------------
{
  function commandLongPayload(command: number, param1: number, targetSystem: number, targetComponent: number): ByteBuffer {
    const payload = ByteBuffer.alloc(33)
...
{
  const positionRequestPayload = (position: number): ByteBuffer => {
    const payload = ByteBuffer.alloc(33)
...
// ---------------------------------------------------------------------------
{
  const messagePayload = (message: Parameters<MavLinkProtocolV2['serialize']>[0]): ByteBuffer =>
...
// ---------------------------------------------------------------------------
{
  ...
  conflictBridge.handleRuntimeCommand({
    type: 'param_set',
  ...
  for (const blocked of [
    ...
    conflictBridge.handleRuntimeCommand(blocked as any, { clientId: 'owner-a' })
    assert.equal(
      findLast(conflictMessages, (message) => message.type === 'operation_error'
...
// safely resolve. It remains fail-closed even after an explicit component pick.
{
  ...
  identityConflictBridge.handleRuntimeCommand({
    type: 'select_target',
...
// ---------------------------------------------------------------------------
{
  ...
  shellOwnedBridge.handleRuntimeCommand({ type: 'shell_open', requestId: 'owned-shell' }, ownerMeta)
  ...
  const probeReply = ByteBuffer.alloc(79)
  probeReply[6] = 10
  probeReply[7] = 1
  probeReply[8] = 5
  ByteBuffer.from('nsh> ', 'ascii').copy(probeReply, 9)
  ...
  shellOwnedBridge.handleRuntimeCommand({ type: 'shell_write', data: { text: 'reboot\r' } }, { clientId: 'client-b' })
  ...
  shellOwnedBridge.handleRuntimeCommand({ type: 'shell_close' }, { clientId: 'client-b' })
  assert.equal(serialFrameCount(), framesBeforeForeignWrite)
  ...
  shellOwnedBridge.handleRuntimeCommand({ type: 'shell_write', data: { text: 'ver hw\r' } }, ownerMeta)
  assert.equal(serialFrameCount(), framesBeforeOwnerWrite + 1)
  shellOwnedBridge.handleRuntimeCommand({ type: 'shell_close' }, ownerMeta)
  assert.equal(lastDelivered('client-a')?.type, 'shell_status')
  ...
  shellOwnedBridge.handleRuntimeCommand({ type: 'shell_open', requestId: 'orphan-shell' }, ownerMeta)
  inject(shellOwnedBridge, 126, probeReply)
