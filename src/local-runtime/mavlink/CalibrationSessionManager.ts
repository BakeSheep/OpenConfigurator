...
// Worker-facing policy around a CalibrationSession: owner identity, the
// owner-only recovery token, bounded orphan recovery, snapshot replay and
// terminal snapshot retention. It never touches the main-thread serial port.
//
...
export interface CalibrationSessionManagerOptions {
  ...
  /** Bind local control authority to the session owner while active. */
