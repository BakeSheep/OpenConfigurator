...
感谢你愿意改进 SkyLab。飞控软件的错误可能造成设备损坏或人身风险，因此本项目对安全边界、回归测试和能力描述的真实性有较高要求。
...
真实硬件测试应记录飞控型号、PX4 版本、操作系统、连接类型、波特率、测试步骤和结果。日志在上传前必须脱敏。尚未做硬件测试时，请明确写出“未做”，不要用单元测试代替硬件结论。
